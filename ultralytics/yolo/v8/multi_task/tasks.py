...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg='yolov8-seg.yaml', ch=3, nc=None, verbose=True):
...
class PoseModel(DetectionModel):
    ...
    def __init__(self, cfg='yolov8-pose.yaml', ch=3, nc=None, data_kpt_shape=(None, None), verbose=True):
...
class multitaskModel(BaseModel):
    def __init__(self, cfg='yolov8-multitask.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes  #TODO modified
        super().__init__()
        self.yaml = cfg if isinstance(cfg, dict) else yaml_model_load(cfg)  # cfg dict
        ...
        if isinstance(m, (Detect, Segment, Pose)):
            ...
            m.inplace = self.inplace
            #forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose)) else self.forward(x)
            detect_out,seg_out,pose_out = self.forward(torch.zeros(1, ch, s, s)) #TODO added            
            m.stride = torch.tensor([s / x.shape[-2] for x in detect_out])  # forward
            self.stride = m.stride
            m.bias_init()  # only run once
        
        # Build strides for detection model 
        m = self.model[self.seg_out_idx]  # Detect() model
        #print(torch.zeros(1, ch, 256, 256).shape,type(torch.zeros(1, ch, 256, 256)))
        if isinstance(m, (Detect, Segment, Pose)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
...
def parse_model_multitask(d, ch, verbose=False):  # model_dict, input_channels(3)
    # Parse a YOLO model.yaml dictionary into a PyTorch model
    import ast
